...
def get_args():
    parser = argparse.ArgumentParser(description="Train VQGraph teacher (GCN/SAGE) and export codebook")
    ...
    parser.add_argument("--teacher", type=str, default="GCN", help="Teacher model: GCN or SAGE")
    parser.add_argument(
...
def run(args):
    ...
    logger.info(
        f"Saved codebook to {output_dir.joinpath('codebook_embeddings.npz')} "
        f"with shape {out_codebook.shape}"
    )
